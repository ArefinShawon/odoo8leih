...
class inventory_item(osv.osv):
    ...
    def validate_phone(self):
        if self.phone:
            match = re.match(r'^01[0-9]{9}$', self.phone)
            if not match:
                raise ValidationError('Provide valid mobile number')

    @api.onchange('email')
    def mail_validation(self):
        if self.email:
            email_match = re.match(r'^[_a-z0-9-]+(\.[_a-z0-9-]+)*@[a-z0-9-]+(\.[a-z0-9-]+)*(\.[a-z]{2,4})$', self.email)
            if not email_match:
